...
(function(){
  'use strict';
  const $=id=>document.getElementById(id),{levels,advance,solve,lowerBound}=LoopBloom,svgNS='http://www.w3.org/2000/svg';
  const storageKey='loop-bloom-v2';
  let saved={scores:{},level:0,sound:false};
  try{const x=JSON.parse(localStorage.getItem(storageKey)||'null');if(x&&typeof x==='object'){saved.scores=x.scores&&typeof x.scores==='object'?x.scores:{};saved.level=Number.isInteger(x.level)?Math.max(0,Math.min(levels.length-1,x.level)):0;saved.sound=x.sound===true;}}catch{}
  ...
  function load(i){
    epoch++;index=Math.max(0,Math.min(levels.length-1,i));level=levels[index];path=[];won=false;dragging=false;hintPath=[];hints=0;saved.level=index;persist();
    ...
    document.querySelectorAll('[data-jump]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.jump)===(index<2?0:index<6?2:6))));
    ...
    status(index<2?'Begin at the ring. The numbers set the order.':'Plan the return path before you commit.');
  ...
  function move(target){
    ...
    if(result.kind==='win'){
      won=true;dragging=false;const score=hints?1:3;saved.scores[index]=Math.max(Number(saved.scores[index])||0,score);persist();$('board').parentElement.classList.add('completed');status('Route complete. Every move accounted for.');draw();tone('win');
    ...
    const count=collected(),left=level.budget-(path.length-1),bound=lowerBound(path.at(-1),count,level);
  ...
  function undo(){if(won||!path.length)return;path.pop();hintPath=[];status(path.length?'Route revised. Keep the return path open.':'Begin at the home ring.');draw();}
  function hint(){
    ...
    if(result.solutions.length){hints++;const route=result.solutions[0];hintPath=route.slice(Math.max(0,prefix.length-1),prefix.length+1);status(path.length?'The dotted segment shows one next move.':'Begin at home, then follow the one dotted segment.');draw();return;}
    for(let keep=prefix.length-1;keep>=1;keep--){const attempt=solve(level,{prefix:prefix.slice(0,keep)});if(attempt.truncated){status('Try undoing a move to find a new route.');return;}if(attempt.solutions.length){hints++;hintPath=[];status('This route cannot finish in '+level.budget+' moves. Undo '+(prefix.length-keep)+' '+(prefix.length-keep===1?'move':'moves')+' and try another direction.');draw();return;}}
  }
  function picker(){
    $('win').close();$('level-grid').replaceChildren();levels.forEach((l,i)=>{const b=document.createElement('button'),s=Number(saved.scores[i])||0;b.className=i===index?'current':'';b.innerHTML=String(i+1).padStart(2,'0')+'<small>'+l.difficulty+'</small><small>'+(s===3?'◆ Mastered':s?'◇ Assisted':l.budget+' moves')+'</small>';b.setAttribute('aria-label',`Puzzle ${i+1}, ${l.difficulty}, ${l.budget} moves, ${s===3?'mastered':s?'assisted':'unsolved'}`);b.addEventListener('click',()=>load(i));$('level-grid').appendChild(b);});$('picker').showModal();
  }
  ...
  $('undo').addEventListener('click',undo);$('reset').addEventListener('click',()=>load(index));$('replay').addEventListener('click',()=>load(index));$('hint').addEventListener('click',hint);
